...
# create file snd.txt
f = open('/Users/sarahtannenbaum/Documents/SAM/Rayon_soundings/snd.txt', 'a+')
...
for i in range(len(time)-1):
    ...
    wind_speed = df['speed'].values * units.meters / units.seconds
    wind_dir = df['direction'].values * units.degrees 
    # calculate wind components, mixing ratios
    u, v = mpcalc.get_wind_components(wind_speed, wind_dir)
    q = mpcalc.saturation_mixing_ratio(p,Td)       
    
    data1 = np.column_stack((z.magnitude, p.magnitude, T.magnitude, 
                q.magnitude*1000, u.magnitude, v.magnitude))
